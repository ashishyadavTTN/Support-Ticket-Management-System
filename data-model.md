...
| Column | Sequelize type | Nullable | Default | Notes |
|--------|----------------|----------|---------|-------|
| `id` | UUID PK | No | UUIDV4 | Migrated from INTEGER → UUID |
| `title` | STRING(255) | No | — | |
...
Beyond the Core happy path, the machine also allows reopen-style moves: `in_progress → open` and `resolved → in_progress`.
...
|--------|----------------|----------|---------|-------|
| `id` | INTEGER PK | No | auto-increment | |
| `ticketId` | UUID FK → `Tickets.id` | No | — | CASCADE |
| `message` | TEXT | No | — | |
...
|--------|----------------|----------|---------|-------|
| `id` | INTEGER PK | No | auto-increment | |
| `ticketId` | UUID FK → `Tickets.id` | No | — | CASCADE delete with ticket |
| `commentId` | INTEGER FK → `Comments.id` | Yes | — | `null` = ticket-level image; set for comment images |
| `uploadedBy` | INTEGER FK → `Users.id` | No | — | |
| `originalName` | STRING(255) | No | — | Client filename |
| `storedName` | STRING(255) | No | — | Filename on disk under `src/uploads/tickets/` |
| `mimeType` | STRING(100) | No | — | `image/jpeg`, `image/png`, `image/gif`, `image/webp` |
| `sizeBytes` | INTEGER | No | — | Max 5 MB per file |
| `createdAt` | DATE | No | — | |
| `updatedAt` | — | — | — | **Disabled** |

Limits (see `constants/attachments.js`): up to **3** images per upload request.
...
Ticket detail responses typically include:

- `creator` / `assignee` (User, attributes: `id`, `name`, `email`, `role`)
- ticket-level `attachments` (`commentId` is null)
- `comments` with nested `author` and comment `attachments`
...
| `20260120000001-create-attachments.js` | `Attachments` table |
| `20260121000001-convert-ticket-id-to-uuid.js` | Ticket (and related FK) INTEGER → UUID |
...
| Concept | Status |
|---------|--------|
| Status history / audit log table | **Not implemented** (`resolvedAt` column only) |
