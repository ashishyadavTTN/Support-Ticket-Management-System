...
Ticket IDs are UUIDs. Image attachments are stored under `src/uploads/tickets/` (gitignored) with metadata in `Attachments`.
...
Core paths: `open → in_progress → resolved → closed`, plus `open|in_progress → cancelled`.  
Also allowed: `in_progress → open`, `resolved → in_progress` (reopen-style). Terminal: `closed`, `cancelled`.
