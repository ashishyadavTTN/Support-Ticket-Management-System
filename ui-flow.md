...
| Page | Route | Description |
|------|-------|-------------|
| Home | `/customer/dashboard` | Status summary cards (from `/dashboard/stats`) + ticket grid |
...
| **Debounced search** | 400ms on ticket list + rep management; inline "Searching…" state |
...
- Read description and assigned agent
- Read/post comments (optimistic UI)
...
- Row click → slide panel (status, assignee, comments)
- New ticket modal (admin can pick customer + assignee)
